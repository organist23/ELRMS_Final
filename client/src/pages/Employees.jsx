...
import { Search, UserPlus, FilePlus, RefreshCw, Send, Trash2, Edit, Users } from 'lucide-react';
...
const Employees = () => {
  ...
  const handleGenerateCredits = async () => {
    if (!window.confirm(`Generate 1.25 VL/SL credits for all employees for ${accrualMonth}/${accrualYear}?`)) return;
    try {
      const { data } = await api.post('/accrual/generate', { month: accrualMonth, year: accrualYear });
      alert(data.message);
      fetchEmployees();
    } catch (err) {
      alert(err.response?.data?.error || 'Failed to generate credits');
    }
  };

  const handleResetPrivileges = async () => {
    if (!window.confirm('Reset ALL employee privilege leaves (Special, Force, Wellness, Solo Parent) to defaults?')) return;
    try {
      const { data } = await api.post('/accrual/reset-privileges');
      alert(data.message);
      fetchEmployees();
    } catch (err) {
      alert('Failed to reset privileges');
    }
  };
    ...
    <div>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <button className="btn-secondary" onClick={async () => {
            const year = prompt('Enter the year to rollover FROM (e.g. 2025):');
            if (year) {
              try {
                await api.post('/accrual/rollover', { from_year: year, to_year: parseInt(year) + 1 });
                alert('Yearly rollover completed successfully.');
                fetchEmployees();
              } catch (err) { alert('Rollover failed'); }
            }
          }} style={{ border: '1px solid var(--primary)', color: 'var(--primary)' }}>
            Yearly Rollover
          </button>
          <button className="btn-secondary" onClick={handleResetPrivileges} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--warning)', borderColor: 'var(--warning)' }}>
            <RefreshCw size={18} />
            Privilege Reset
          </button>
      ...
      <div className="premium-card" style={{ marginBottom: '32px', display: 'flex', alignItems: 'center', gap: '24px', background: 'var(--primary-light)', border: 'none' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <input type="number" className="input-field" style={{ width: '80px', padding: '8px' }} value={accrualMonth} onChange={e => setAccrualMonth(e.target.value)} placeholder="Month" />
          <input type="number" className="input-field" style={{ width: '100px', padding: '8px' }} value={accrualYear} onChange={e => setAccrualYear(e.target.value)} placeholder="Year" />
          <button className="btn-primary" style={{ padding: '8px 16px' }} onClick={handleGenerateCredits}>Confirm Generate</button>
        </div>
      </div>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                   <button className="icon-btn" style={{ color: 'var(--secondary)' }}><Edit size={18} /></button>
                   <button className="icon-btn" style={{ color: 'var(--danger)' }}><Trash2 size={18} /></button>
                </div>
              </div>
              ...
              </div>              <h4 style={{ fontSize: '1rem', fontWeight: '700', marginBottom: '16px', color: 'var(--primary)', borderBottom: '1px solid var(--border)', paddingBottom: '8px' }}>Leave Credits</h4>
