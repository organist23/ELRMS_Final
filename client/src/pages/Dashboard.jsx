...
const Dashboard = () => {
  const [stats, setStats] = useState({ totalEmployees: 0, pendingApproval: 0, activeOnLeave: 0 });
  ...
  return (
    <div>
      ...
      <div className="stat-grid">
        ...
        <div className="premium-card stat-card">
          <div className="icon-box icon-green"><Activity size={24} /></div>
          <div>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', fontWeight: '500' }}>Active On Leave</p>
            <h3 style={{ fontSize: '1.5rem', fontWeight: '700' }}>{stats.activeOnLeave}</h3>
          </div>
        </div>
      </div>
              ...
              {recentLedger.length > 0 ? recentLedger.map((item) => (
                <tr key={item.id}>
                  ...
                  <td style={{ fontSize: '0.875rem' }}>
                    <div style={{ fontWeight: '500', color: 'var(--primary)', marginBottom: '4px' }}>{item.transaction_desc}</div>
                    <div style={{ display: 'flex', gap: '8px', fontSize: '0.7rem', color: 'var(--text-muted)' }}>
                      <span>VL: {parseFloat(item.vl_bal).toFixed(1)}</span>
                      <span>SL: {parseFloat(item.sl_bal).toFixed(1)}</span>
                    </div>
                  </td>
                  <td>
